      ...
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({
          ...
          eventDetails: {
            ...
            closing_paragraph: data.closingParagraph, institution_full: "",
          },
        }),
      });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-red-400/20 to-rose-500/20 border border-red-500/30 flex items-center justify-center">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <Card className="border border-white/10 bg-card/50">
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2"><Label>Event Name</Label><Input placeholder="e.g. TECHKRITI 2026" {...register("eventName")} className="border-white/15" /></div>
            <div className="space-y-2"><Label>Organised By</Label><Input placeholder="e.g. IIT Kanpur" {...register("eventHost")} className="border-white/15" /></div>
            <div className="space-y-2"><Label>Event Dates</Label><Input placeholder="e.g. 19 to 22 March 2026" {...register("eventDatesFull")} className="border-white/15" /></div>
            <div className="space-y-2"><Label>Month / Year</Label><Input placeholder="e.g. March 2026" {...register("monthYear")} className="border-white/15" /></div>
          </CardContent>
        </Card>
        ...
        <Card className="border border-white/10 bg-card/50">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2"><Label>Introduction Paragraph</Label><Textarea placeholder="Brief introduction about the event and team's participation..." {...register("introParagraph")} rows={4} className="border-white/15 resize-none" /></div>
            <div className="space-y-2"><Label>Results & Achievements</Label><Textarea placeholder="Describe the results, achievements, and competition details..." {...register("resultsSection")} rows={5} className="border-white/15 resize-none" /></div>
            <div className="space-y-2"><Label>Closing Paragraph</Label><Textarea placeholder="Closing remarks, acknowledgements..." {...register("closingParagraph")} rows={3} className="border-white/15 resize-none" /></div>
          </CardContent>
        </Card>
